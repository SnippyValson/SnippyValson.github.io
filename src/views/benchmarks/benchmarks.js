...
for(let i =0; i< numPhysicalThreads; i++){
    var w = new Worker("bubblesort_worker.js");
    ...
    var qw = new Worker("quicksort_worker.js");
...
function bubbleSortArray() {
    ...
    bubbleSortWorker.postMessage({ array : array });
}
...
function quickSortArray() {
    document.getElementById("info-label").innerHTML = "Loading...";
    startTime = performance.now();
    quickSortWorker.postMessage({ array : array });
}
...
function multiBubbleSortArray() {
    ...
    for(let i = 0; i < numPhysicalThreads; i++) {
        bubbleSortWorkers[i].postMessage({array : array.slice(index, index + sliceLength)});
        index += sliceLength;
    }
}
...
function multiQuickSortArray() {
    ...
    let sliceLength = arraySize / numPhysicalThreads;
    for(let i = 0; i < numPhysicalThreads; i++) {
        quickSortWorkers[i].postMessage({array : array.slice(index, index + sliceLength)});
        index += sliceLength;
    }
}
